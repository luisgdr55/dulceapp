...
router.get('/', requireWorkspace('VIEWER'), async (req, res) => {
  ...
  // Stock bajo — post-query JS (SQLite no soporta comparación entre columnas en WHERE)
  const ingredientesAll = await prisma.ingrediente.findMany({
    where: { workspaceId: wid },
    select: { id: true, nombre: true, cantidadActual: true, cantidadMinima: true, unidad: true }
  })
  const stockBajo = ingredientesAll
    .filter(i => i.cantidadActual <= i.cantidadMinima)
    .sort((a, b) => (a.cantidadActual / Math.max(a.cantidadMinima, 0.001)) - (b.cantidadActual / Math.max(b.cantidadMinima, 0.001)))
    .slice(0, 5)
  ...
  const recetasInfo = recetaIds.length > 0
    ? await prisma.receta.findMany({ where: { id: { in: recetaIds } }, select: { id: true, nombre: true } })
    : []
