// src/ai/tools.js
// Implementación real de cada tool que puede llamar el LLM
// Cada función valida params con Zod, ejecuta en DB y registra en AuditLog
...
async function crearPedido(params, { workspaceId, userId, canal }) {
  ...
  // Obtener tasa BCV actual y receta
  ...
  const variante = data.varianteId
    ? receta.variantes.find(v => v.id === data.varianteId)
    : null
  ...
  if (receta.ingredientes) {
    for (const item of receta.ingredientes) {
      await prisma.ingrediente.update({
        ...
        data: { cantidadActual: { decrement: item.cantidad * data.cantidad } }
      })
    }
  }
  ...
  await prisma.auditLog.create({
    data: { workspaceId, userId, canal, accion: 'crear_pedido', entidad: 'Pedido', entidadId: pedido.id, payload: pedido }
  })

  return {
    ok: true,
    pedido: { id: pedido.id, numeroPedido, totalEur, totalBs, estado: 'PENDIENTE' }
  }
}
...
async function actualizarStock(params, { workspaceId, userId, canal }) {
  ...
  await prisma.auditLog.create({
    data: { workspaceId, userId, canal, accion: 'actualizar_stock', entidad: 'Ingrediente', entidadId: ingrediente.id, payload: { nuevaCantidad: data.nuevaCantidad, motivo: data.motivo } }
  })
...
async function agregarIngrediente(params, { workspaceId, userId, canal }) {
  ...
  await prisma.auditLog.create({
    data: { workspaceId, userId, canal, accion: 'agregar_ingrediente', entidad: 'Ingrediente', entidadId: ingrediente.id, payload: ingrediente }
  })
...
async function actualizarTasaBCV(params, { workspaceId, userId, canal }) {
  ...
  // Desactivar tasa anterior y crear la nueva
  ...
  await prisma.auditLog.create({
    data: { workspaceId, userId, canal, accion: 'actualizar_tasa_bcv', entidad: 'TasaBCV', payload: { tasa } }
  })
...
async function proyectarDemanda({ diasProyeccion = 7 }, { workspaceId }) {
  ...
  // Calcular consumo promedio diario por ingrediente
  const consumoDiario = {}
  ...
  const proyeccion = await Promise.all(
    Object.values(consumoDiario).map(async (item) => {
      ...
      const actual = await prisma.ingrediente.findUnique({
        where: { id: item.id },
...
async function resumenNegocio({ periodo = '7d' }, { workspaceId }) {
  ...
  const [pedidosEntregados, pedidosPendientes, totalIngredientes] = await Promise.all([
    ...
    prisma.ingrediente.count({ where: { workspaceId, cantidadActual: { lte: prisma.ingrediente.fields.cantidadMinima } } })
  ])
  ...
  return {
    ...
    pedidosPendientes,
    ingredientesBajoStock: totalIngredientes
  }
}
