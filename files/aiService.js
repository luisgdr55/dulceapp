// src/ai/aiService.js
// Corazón del sistema IA — Gemini Flash via OpenRouter con tool calling real
// Usado tanto por la PWA (web) como por el bot de Telegram
...
// ─── Definición de tools para Gemini (OpenAI-compatible tool calling) ────────
const TOOLS = [
  {
    type: 'function',
    function: {
...
export async function processAIMessage({ workspaceId, userId, canal, userMessage, conversationId }) {
  ...
  // 3. Construir system prompt con contexto en tiempo real del negocio
  ...
  // 5. Procesar la respuesta — puede tener tool calls
  ...
  await prisma.aIConversation.update({
    ...
    data: { messages: result.updatedMessages, updatedAt: new Date() }
  })

  return {
    conversationId: conversation.id,
    reply: result.reply,
    toolActions: result.toolActions,    // acciones pendientes de confirmación
    executedActions: result.executedActions  // acciones ya ejecutadas automáticamente
  }
}
...
async function processResponse({ response, workspaceId, userId, canal, conversationId, messages }) {
  ...
  if (assistantMessage.tool_calls?.length > 0) {
    ...
    for (const toolCall of assistantMessage.tool_calls) {
      ...
      // Clasificar: ¿esta tool necesita confirmación o se ejecuta automáticamente?
      const needsConfirmation = ['crear_pedido', 'venta_rapida', 'actualizar_stock'].includes(toolName)

      if (needsConfirmation) {
        // Registrar como acción pendiente — el usuario confirma desde la UI
        ...
        // Ejecutar directamente (consultas de lectura, análisis)
    ...
    // Segunda llamada con los resultados de las tools para generar respuesta final
    updatedMessages = [...updatedMessages, ...toolResults]
    const secondResponse = await callOpenRouter(
      await buildSystemPrompt(workspaceId),
      updatedMessages.slice(1) // sin el system (ya va en el primer arg)
    )
...
// ─── Confirmar una acción pendiente desde la UI ──────────────────────────────
export async function confirmAIAction({ actionId, workspaceId, userId }) {
  ...
  if (!action || action.ejecutada) {
    throw new Error('Acción no encontrada o ya ejecutada')
  }

  if (action.conversation.workspaceId !== workspaceId) {
    throw new Error('Sin acceso a esta acción')
  }
  ...
  await prisma.aIAction.update({
    where: { id: actionId },
    data: { confirmada: true, ejecutada: true, resultado }
  })
