...
export async function buildSystemPrompt(workspaceId) {
  // Cargar contexto del negocio en paralelo
  const [config, tasaActual, recetas, stockBajo, pedidosPendientes] = await Promise.all([
    prisma.businessConfig.findUnique({ where: { workspaceId } }),

    prisma.tasaBCV.findFirst({
      where: { workspaceId, esCurrent: true },
      orderBy: { fecha: 'desc' }
    }),
    ...
    prisma.ingrediente.findMany({
      where: {
        workspaceId,
        cantidadActual: { lte: prisma.ingrediente.fields.cantidadMinima }
      },
